...
(function(){
  ...
  async function load(){
    ...
    try{
      ...
      let imgs = Array.isArray(p.images)&&p.images.length ? p.images : [p.imageUrl || (Array.isArray(p.images)&&p.images[0])].filter(Boolean);
      let vids = Array.isArray(p.videos)&&p.videos.length ? p.videos : (p.videoUrl ? [p.videoUrl] : []);
      // Normalisasi path untuk GitHub Pages (project site) agar tidak menuju domain root
      (function(){
        const isGh = location.hostname.endsWith('github.io');
        const project = location.pathname.split('/')[1] || '';
        const base = isGh ? `/${project}/` : '/';
        const normalize = (u) => {
          if (!u) return '';
          if (/^https?:\/\//i.test(u)) return u; // sudah absolut
          if (u.startsWith('/')) return base + u.slice(1); // absolut ke domain root → ke base project
          return base + u; // relatif → pastikan berawalan base project
        };
        imgs = (imgs||[]).map(normalize);
        vids = (vids||[]).map(normalize);
        // Simpan ke scope agar bisa dipakai di bawah
        window.__normalizeAssetPath = normalize;
      })();
      ...
      // Preload gambar utama untuk mempercepat first paint di perangkat mobile
      try {
        if (imgs && imgs[0]) {
  ...
  const waMsg = encodeURIComponent(`Halo Admin SDL KNIVES, saya minat ${p.name} (ID: ${p.id}). Mohon konsultasi gratis untuk personalisasi bilah & sarung.\nHarga: ${rupiah(p.price)}`);
      ...
      container.innerHTML = `
        ...
        <div class="product-container">
          <div class="gallery">
            <img class="main zoomable loading" src="${imgs[0]||''}" alt="${p.name}" loading="eager" decoding="async" fetchpriority="high" />
            ...
            <div class="actions">
  <a class="btn btn-primary" target="_blank" rel="noopener noreferrer" href="https://wa.me/6287767896317?text=${waMsg}"><i class="bi bi-whatsapp"></i> Beli via WA</a>
      ...
      if (main && main.tagName.toLowerCase() === 'img') {
        main.addEventListener('load', ()=> main.classList.remove('loading'));
      }
      // Prefetch gambar di sekitar indeks saat ini untuk mempercepat pergantian
      function prefetchAround(idx){
        const preload = [ imgs[(idx+1)%imgs.length], imgs[(idx-1+imgs.length)%imgs.length] ].filter(Boolean);
        preload.forEach(src=>{ try{ const im=new Image(); im.loading='lazy'; im.decoding='async'; im.src=src; }catch{} });
      }
      prefetchAround(0);
      ...
      function updateActiveThumb(){
        try{
          ...
          const cur = thumbs[currentIndex];
          if(cur){
            cur.classList.add('active');
            const behavior = prefersReduced ? 'auto' : 'smooth';
            cur.scrollIntoView({ behavior, inline: 'center', block: 'nearest' });
          }
        }catch{}
      }
      thumbs.forEach((t,idx)=>t.addEventListener('click',()=>{ const m = container.querySelector('.gallery .main'); if(m && m.tagName.toLowerCase()==='img'){ m.classList.add('loading'); m.src = t.dataset.src; m.classList.add('zoomable'); currentIndex = idx; prefetchAround(currentIndex); m.addEventListener('load', ()=> { m.classList.remove('loading'); updateActiveThumb(); }, { once: true }); } else if(m){
        const img = document.createElement('img'); img.className='main zoomable loading'; img.alt = p.name; img.src = t.dataset.src; img.loading='eager'; img.decoding='async'; img.setAttribute('fetchpriority','high'); m.parentElement.replaceChild(img, m); currentIndex = idx; img.addEventListener('click',()=>openLightbox(currentIndex)); img.addEventListener('load', ()=> img.classList.remove('loading'), { once: true });
      }}));
      function setMainImage(idx){
        const m = container.querySelector('.gallery .main');
        const src = imgs[idx]; currentIndex = idx;
        if(m && m.tagName.toLowerCase()==='img'){ m.classList.add('loading'); m.src = src; m.classList.add('zoomable'); prefetchAround(currentIndex); m.addEventListener('load', ()=> { m.classList.remove('loading'); updateActiveThumb(); }, { once: true }); }
        else if(m){ const img = document.createElement('img'); img.className='main zoomable loading'; img.alt = p.name; img.src = src; img.loading='eager'; img.decoding='async'; img.setAttribute('fetchpriority','high'); m.parentElement.replaceChild(img, m); img.addEventListener('click',()=>openLightbox(currentIndex)); img.addEventListener('load', ()=> { img.classList.remove('loading'); updateActiveThumb(); }, { once: true }); }
      }
      // Saat idle, prefetch semua gambar untuk kelancaran selanjutnya
      const idle = window.requestIdleCallback || ((fn)=>setTimeout(fn,500));
      idle(()=>{ try{ imgs.slice(1).forEach(src=>{ const im=new Image(); im.loading='lazy'; im.decoding='async'; im.src=src; }); }catch{} });
      ...
      // Swipe gesture untuk perangkat mobile: geser kiri/kanan pada gambar utama
      (function(){
        const gallery = container.querySelector('.gallery');
        const surface = gallery?.querySelector('.main') || gallery;
        if (!surface) return;
        let startX = 0, startY = 0, deltaX = 0, swiping = false;
        const threshold = 48; // piksel minimal untuk berpindah
        const onStart = (x, y) => { startX = x; startY = y; deltaX = 0; swiping = false; };
        const onMove = (x, y, ev) => {
          const dx = x - startX; const dy = y - startY; deltaX = dx;
          if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 14) { swiping = true; ev?.preventDefault?.(); }
        };
        const onEnd = () => {
          if (swiping && Math.abs(deltaX) > threshold) {
            if (deltaX < 0) { setMainImage((currentIndex+1)%imgs.length); }
            else { setMainImage((currentIndex-1+imgs.length)%imgs.length); }
          }
          startX = 0; startY = 0; deltaX = 0; swiping = false;
        };
        // Touch events
        surface.addEventListener('touchstart', (e)=>{ const t = e.changedTouches[0]; onStart(t.clientX, t.clientY); }, { passive: true });
        surface.addEventListener('touchmove', (e)=>{ const t = e.changedTouches[0]; onMove(t.clientX, t.clientY, e); }, { passive: false });
        surface.addEventListener('touchend', onEnd);
        // Pointer events fallback
        surface.addEventListener('pointerdown', (e)=>{ if (e.pointerType === 'mouse') return; onStart(e.clientX, e.clientY); });
        surface.addEventListener('pointermove', (e)=>{ if (e.pointerType === 'mouse') return; onMove(e.clientX, e.clientY, e); });
        surface.addEventListener('pointerup', (e)=>{ if (e.pointerType === 'mouse') return; onEnd(); });
      })();
      ...
      function openLightbox(idx){
        if (isMobile) return; // nonaktifkan lightbox di perangkat kecil agar tidak menghalangi
        currentIndex = idx;
      ...
      if(main && main.tagName.toLowerCase()==='img' && !isMobile){ main.addEventListener('click',()=>openLightbox(currentIndex)); }

      // Gesture swipe kiri/kanan untuk ganti gambar di mobile
      if (main) {
        let sx = 0, sy = 0; let dx = 0, dy = 0;
        const TH = 40; const VMAX = 80;
        main.addEventListener('touchstart', (e)=>{ const t=e.changedTouches[0]; sx=t.pageX; sy=t.pageY; dx=0; dy=0; }, {passive:true});
        main.addEventListener('touchmove', (e)=>{ const t=e.changedTouches[0]; dx=t.pageX-sx; dy=t.pageY-sy; }, {passive:true});
        main.addEventListener('touchend', ()=>{
          if (Math.abs(dx) > TH && Math.abs(dy) < VMAX) {
            if (dx > 0) setMainImage((currentIndex-1+imgs.length)%imgs.length);
            else setMainImage((currentIndex+1)%imgs.length);
          }
        }, {passive:true});
      }
      ...
      if(bar){
        bar.innerHTML = `<div class="container inner"><div class="name">${p.name}</div><div class="price">${rupiah(p.price)}</div><div class="actions"><a class="btn btn-primary" target="_blank" href="https://wa.me/6287767896317?text=${waMsg}"><i class="bi bi-whatsapp"></i> Konsultasi & Beli</a></div></div>`;
      ...
      if(fbtEl && fbtTotalEl && fbtBtn){
        ...
        const norm = window.__normalizeAssetPath || ((u)=>u);
        fbtEl.innerHTML = accessories.map((x,i)=>{
          const img = norm(x.imageUrl || (Array.isArray(x.images)? x.images[0] : ''));
      ...
      if (relatedEl) {
        const related = (list||[])
          ...
          .slice(0, 4);
        const norm2 = window.__normalizeAssetPath || ((u)=>u);
        relatedEl.innerHTML = related.map((x,i)=>{
          const img = norm2(x.imageUrl || (Array.isArray(x.images) ? x.images[0] : ''));
          return `
